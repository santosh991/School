...
public class TestCat2DAO {
	...
	private double CAT2 = 15;
	
	
	private Cat2DAO store;

	/**
	 * Test method for {@link com.yahoo.petermwenda83.persistence.exam.Cat2DAO#hasCat2(com.yahoo.petermwenda83.bean.exam.result.Perfomance)}.
	 */
	@Ignore
	@Test
	public void testHasCat2() {
		store = new Cat2DAO(databaseName, Host, databaseUsername, databasePassword, databasePort);
		Perfomance p = new Perfomance();
		p.setStudentUuid(STU_UUID);
		p.setExamDetailUuid(EXAM_DETAIL_UUID); 
		assertTrue(store.hasCat2(p)); 
	}

	/**
	 * Test method for {@link com.yahoo.petermwenda83.persistence.exam.Cat2DAO#putCat2(com.yahoo.petermwenda83.bean.exam.result.Perfomance, double)}.
	 */
	//@Ignore
	@Test
	public void testPutCat2() {
		store = new Cat2DAO(databaseName, Host, databaseUsername, databasePassword, databasePort);
		Perfomance p = new Perfomance();
		p.setUuid(UUID);
		...
		assertTrue(store.putCat2(p, CAT2)); 
	}
